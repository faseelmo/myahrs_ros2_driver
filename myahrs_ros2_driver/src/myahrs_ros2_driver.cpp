...
namespace WithRobot
{
  ...
  bool MyAhrsDriverForROS::initialize()
  {
    ...
    do
    {
      ...
      if (cmd_binary_data_format("EULER, IMU") == false)
      {
        break;
      }
      // 100Hz
      if (cmd_divider("1") == false)
      {
        break;
      }
  ...
  void MyAhrsDriverForROS::publish_topic(int sensor_id)
  {
    ...
    // original sensor data used the degree unit, convert to radian (see ROS
    // REP103) we used the ROS's axes orientation like x forward, y left and z
    // up so changed the y and z aixs of myAHRS+ board
